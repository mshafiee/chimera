    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <MetricCard
          ...
          value={`${((data.consensus_rate ?? 0) * 100).toFixed(1)}%`}
          positive={(data.consensus_rate ?? 0) > 0.7}
          icon={<Network className="w-4 h-4" />}
        />
        <MetricCard
          label="Avg Clustering"
          value={(data.avg_clustering_coefficient ?? 0).toFixed(2)}
          icon={<Users className="w-4 h-4" />}
        />
        <MetricCard
          label="Divergence Alerts"
          value={data.divergence_alerts?.length ?? 0}
          positive={(data.divergence_alerts?.length ?? 0) === 0}
      ...
      {data.active_clusters?.length > 0 && (
        <div>
          <h3 className="text-sm font-medium mb-3">Active Clusters ({data.active_clusters?.length ?? 0})</h3>
            ...
            {data.active_clusters?.slice(0, 6).map((cluster) => (
              <div key={cluster.id} className="bg-surface-light rounded-lg p-3">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span>{cluster.wallets?.length ?? 0} wallets</span>
                  <span className="text-text-muted">Coherence: {(cluster.coherence ?? 0).toFixed(2)}</span>
                </div>
                <div className="text-xs text-text-muted mt-1">
                  Avg WQS: {(cluster.avg_wqs ?? 0).toFixed(1)}
                </div>
              </div>
            ))}
          </div>
