    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-surface-light rounded-lg p-4">
          ...
          <div className="text-xl font-semibold font-mono-numbers">
            {data.aggregated_signals.length}
          </div>
        </div>
        <div className="bg-surface-light rounded-lg p-4">
          <div className="text-xs text-text-muted mb-1">Aggregation Latency</div>
          <div className="text-xl font-semibold font-mono-numbers">
            {data.aggregation_latency_ms.toFixed(0)}ms
          </div>
        </div>
      </div>
          ...
          {data.aggregated_signals.map((signal, index) => {
            ...
            return (
              <TableRow key={`${signal.token_address}-${index}`}>
                ...
                <TableCell mono className="text-sm text-right">
                  <span className={signal.consensus_score >= 0.7 ? 'text-profit' : signal.consensus_score >= 0.5 ? 'text-spear' : 'text-loss'}>
                    {signal.consensus_score.toFixed(2)}
                  </span>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-2 bg-surface rounded-full overflow-hidden">
                      <div
                        className={`h-full ${
                          signal.confidence >= 0.7 ? 'bg-profit' : signal.confidence >= 0.5 ? 'bg-spear' : 'bg-loss'
                        }`}
