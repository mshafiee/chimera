    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-surface-light rounded-lg p-4">
          ...
          <div className="text-xl font-semibold font-mono-numbers">
            {(data.clustering_metrics?.avg_cluster_size ?? 0).toFixed(1)}
          </div>
        </div>
        <div className="bg-surface-light rounded-lg p-4">
          <div className="text-xs text-text-muted mb-1">Silhouette Score</div>
          <div className="text-xl font-semibold font-mono-numbers">
            {(data.clustering_metrics?.silhouette_score ?? 0).toFixed(2)}
          </div>
        </div>
      </div>
        ...
        <TableBody>
          {data.clusters?.map((cluster) => (
            <TableRow key={cluster.id}>
              ...
              <TableCell mono className="text-sm text-right">
                <span className={(cluster.avg_wqs ?? 0) >= 60 ? 'text-profit' : (cluster.avg_wqs ?? 0) >= 40 ? 'text-spear' : 'text-loss'}>
                  {(cluster.avg_wqs ?? 0).toFixed(1)}
                </span>
              </TableCell>
              <TableCell>
                <Badge
                  variant={(cluster.coherence ?? 0) >= 0.7 ? 'success' : (cluster.coherence ?? 0) >= 0.5 ? 'warning' : 'default'}
                  size="sm"
                >
                  {(cluster.coherence ?? 0).toFixed(2)}
                </Badge>
              </TableCell>
