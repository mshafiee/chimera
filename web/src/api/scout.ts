...
export function useScoutStatus(refetchInterval?: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<ScoutStatusResponse>('/api/v1/scout/status')
...
export function useWQSDistribution(timeRange?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<WQSDistributionResponse>('/api/v1/scout/wqs-distribution', {
...
export function useScoutMetrics(timeRange?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<ScoutMetricsResponse>('/api/v1/scout/metrics', {
        params: timeRange ? { range: timeRange } : undefined,
      })
      return response.data
    },
...
export async function triggerScoutRun(): Promise<{ run_id: string; scheduled_at: string }> {
  const response = await apiClient.post<{ run_id: string; scheduled_at: string }>('/api/v1/scout/run')
  return response.data
}
