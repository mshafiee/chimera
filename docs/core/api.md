...
Returns system resource usage metrics (CPU, memory, disk, network).

**Authentication:** None (public endpoint for dashboard)
...
Returns secret rotation status and history.
...
```json
{
  ...
  "next_rotation_at": "2025-04-01T00:00:00Z",
  "days_until_due": 75,
  "status": "active",
  "rotation_history": [
    {
...
**Status values:** `active`, `due_soon`, `overdue`, `unknown`

**Event status values:** `success`, `failed`, `partial`

#### GET /api/v1/operations/rate-limit
Returns API rate limiting status per endpoint.
...
**Overall status values:** `healthy`, `degraded`, `throttled`

**Endpoint status values:** `ok`, `warning`, `throttled`

#### GET /api/v1/operations/health-checks
Returns detailed system health check results with response times.
...
```json
{
  ...
  "checks": [
    ...
    },
    {
      ...
      "status": "passing",
      "message": "Circuit breaker active, trading allowed",
      "last_check": "2025-01-15T10:30:00Z",
    ...
    },
    {
      "name": "price_cache",
      "status": "passing",
      "message": "Price cache healthy: 150 entries, 25 tracked tokens",
      "last_check": "2025-01-15T10:30:00Z",
...
**Check status values:** `passing`, `warning`, `failing`
