...
notify_failure() {
    ...
    if [[ "$NOTIFY_ON_FAILURE" == "true" ]]; then
        if [[ -n "${TELEGRAM_BOT_TOKEN:-}" && -n "${TELEGRAM_CHAT_ID:-}" ]]; then
            # Escape HTML-sensitive characters so parse_mode=HTML does not reject the message
            local escaped
            escaped=$(echo "$message" | sed 's/&/\&amp;/g; s/</\&lt;/g; s/>/\&gt;/g')
            curl -s --max-time 10 -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
                -d "chat_id=${TELEGRAM_CHAT_ID}" \
                --data-urlencode "text=🚨 Chimera Backup Failed: ${escaped}" \
...
# The path may contain single quotes (from CHIMERA_HOME); escape them.
SAFE_BACKUP_FILE=$(sql_escape "$BACKUP_FILE")
log "INFO" "Running VACUUM INTO ${BACKUP_FILE}"
if ! sqlite3 "$DB_PATH" "VACUUM INTO '${SAFE_BACKUP_FILE}';" 2>> "$LOG_FILE"; then
...
# Verify backup integrity (capture the exit status so set -e cannot mask it)
if ! INTEGRITY_CHECK=$(sqlite3 "$BACKUP_FILE" "PRAGMA integrity_check;" 2>&1) || [[ "$INTEGRITY_CHECK" != "ok" ]]; then
    notify_failure "Backup integrity check failed: $INTEGRITY_CHECK"
    rm -f "$BACKUP_FILE"
    exit 1
fi

# Optional: Compress backup if larger than 100MB (do this BEFORE recording
# metadata so the recorded path/size/checksum match the actual artifact)
...
# Record backup in database (with metadata of the final artifact)
...
VALUES ('${SAFE_BACKUP_FILE}', ${BACKUP_SIZE:-0}, '${CHECKSUM}', 'SCHEDULED', datetime('now'));
EOF
then
    notify_failure "Failed to record backup in database"
    rm -f "$BACKUP_FILE"
    exit 1
fi

log "INFO" "Backup recorded: ${BACKUP_FILE} (${BACKUP_SIZE} bytes, SHA256: ${CHECKSUM:0:16}...)"

# Cleanup old backups (older than RETENTION_DAYS), including compressed ones
...
done < <(find "$BACKUP_DIR" \( -name "chimera_*.db" -o -name "chimera_*.db.gz" \) -type f -mtime +${RETENTION_DAYS} 2>/dev/null)
