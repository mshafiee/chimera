...
async fn test_helius_websocket_connection() -> Result<()> {
    ...
    // Never print the API key to logs.
    println!("Connecting to: wss://mainnet.helius-rpc.com/?api-key=<redacted>");
    ...
    // LaserStream (Jito-compatible) transactionSubscribe filters use
    // accountInclude/accountExclude, not `account`.
    let subscription = json!({
        "jsonrpc": "2.0",
        "id": 1,
        "method": "transactionSubscribe",
        "params": [{
            "accountInclude": [test_wallet],
            "failed": false,
                    ...
                    match message {
                        tokio_tungstenite::tungstenite::Message::Text(text) => {
                            ...
                            if let Ok(value) = serde_json::from_str::<serde_json::Value>(&text) {
                                ...
                                // LaserStream emits `transactionNotification` for
                                // transactionSubscribe notifications.
                                if value.get("method").and_then(|m| m.as_str())
                                    == Some("transactionNotification")
                                {
                                    println!("  ✅ Got transaction notification!");
                                    message_count += 1;
                                }
                            }
                        }
    ...
    match message_timeout {
        Ok(Ok(count)) => {
            if count > 0 {
                println!("✅ Received {count} messages in 30 seconds");
                println!("✅ WebSocket test PASSED");
            } else {
                return Err(anyhow!(
                    "No transaction messages received within 30 seconds (wallet may be inactive)"
                ));
            }
        }
        Ok(Err(e)) => return Err(anyhow!("WebSocket test failed: {e}")),
        Err(_) => {
            return Err(anyhow!(
                "Timeout after 30 seconds - no recent transactions for test wallet"
            ));
        }
    }
...
async fn test_websocket_ping_pong() -> Result<()> {
    ...
    println!("Testing ping/pong with: wss://mainnet.helius-rpc.com/?api-key=<redacted>");
    ...
    if pong_received == Ok(true) {
        println!("✅ Received pong - ping/pong working!");
    } else {
        return Err(anyhow!("No pong received within 5 seconds"));
    }
