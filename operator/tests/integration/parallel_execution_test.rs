...
fn make_config() -> AppConfig {
    let builder = config::Config::builder()
        ...
        .set_default("queue.num_workers", 4).unwrap()
        .set_default("queue.max_concurrent_rpc", 8).unwrap()
...
async fn make_processor(config: &Arc<AppConfig>) -> (SignalProcessor, Arc<dyn chimera_operator::db_abstraction::Database>) {
...
async fn test_five_concurrent_signals_process_in_parallel() {
    ...
    assert_eq!(
        stats.queue_depth, 0,
        "All 5 signals should be dequeued, took {:?}", elapsed
    );
    assert!(
...
async fn test_priority_ordering_under_concurrent_load() {
    ...
    queue.push(make_signal(1, Strategy::Spear, "SPEAR1"), None).await.unwrap();
    queue.push(make_signal(2, Strategy::Shield, "SHIELD1"), None).await.unwrap();
    queue.push(make_signal(3, Strategy::Exit, "EXIT1"), None).await.unwrap();
    queue.push(make_signal(4, Strategy::Spear, "SPEAR2"), None).await.unwrap();
    queue.push(make_signal(5, Strategy::Shield, "SHIELD2"), None).await.unwrap();

    assert_eq!(queue.pop().await.unwrap().payload.strategy, Strategy::Exit, "EXIT must be first");
    assert_eq!(queue.pop().await.unwrap().payload.strategy, Strategy::Shield, "SHIELD must be second");
    assert_eq!(queue.pop().await.unwrap().payload.strategy, Strategy::Shield, "SHIELD must be third");
    assert_eq!(queue.pop().await.unwrap().payload.strategy, Strategy::Spear, "SPEAR must be fourth");
    assert_eq!(queue.pop().await.unwrap().payload.strategy, Strategy::Spear, "SPEAR must be fifth");
...
async fn test_graceful_shutdown_with_in_flight_signals() {
    ...
    for i in 0..3 {
        queue.push(make_signal(i, Strategy::Shield, &format!("TOKEN{}", i)), None).await.unwrap();
    }
