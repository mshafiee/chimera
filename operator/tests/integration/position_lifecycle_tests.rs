...
use chimera_operator::db::{
    ...
    insert_trade, update_trade_status, open_position, close_position,
    trade_uuid_exists,
};
...
async fn test_duplicate_buy_uuid_idempotency() {
    ...
    insert_trade(&pool, uuid, "wallet", "token", Some("T"), "SHIELD", "BUY",
        Decimal::from_str("1.0").unwrap(), "PENDING").await.unwrap();
    open_position(&pool, uuid, "wallet", "token", Some("T"), "SHIELD",
        Decimal::from_str("1.0").unwrap(), Decimal::from_str("1.0").unwrap(), "sig1").await.unwrap();

    // Second insert for the same UUID must fail (UNIQUE violation)
    let second_insert = insert_trade(&pool, uuid, "wallet", "token", Some("T"), "SHIELD", "BUY",
        Decimal::from_str("1.0").unwrap(), "PENDING").await;
    assert!(second_insert.is_err(), "Duplicate trade_uuid must be rejected by UNIQUE constraint");
    ...
    let pos_count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM positions WHERE trade_uuid = ?")
        ...
        .await
        .unwrap();
    assert_eq!(pos_count.0, 1, "Exactly one position must exist for a duplicated trade_uuid");
}
...
async fn test_close_position_no_active_position_is_noop() {
    ...
    let result = close_position(
        &pool, "token_nosell", "wallet_nosell",
        Decimal::from_str("2.0").unwrap(), "sig_exit",
    ).await;

    assert!(result.is_ok(), "Closing non-existent position should not error");

    let pos_count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM positions")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(pos_count.0, 0, "No positions should exist after close on empty DB");
}
...
async fn test_pnl_calculation_accuracy_with_fees() {
    ...
    insert_trade(&pool, uuid, "wallet_f", "token_f", Some("F"), "SHIELD", "BUY",
        Decimal::from_str("1.0").unwrap(), "ACTIVE").await.unwrap();
    open_position(&pool, uuid, "wallet_f", "token_f", Some("F"), "SHIELD",
        Decimal::from_str("1.0").unwrap(),
        Decimal::from_str("100.0").unwrap(), // entry $100
        "sig_buy_f").await.unwrap();

    // Sell at $110
    close_position(&pool, "token_f", "wallet_f",
        Decimal::from_str("110.0").unwrap(), "sig_sell_f").await.unwrap();

    let (realized_pnl,): (f64,) = sqlx::query_as(
        "SELECT realized_pnl_sol FROM positions WHERE trade_uuid = ?"
    )
    .bind(uuid)
    .fetch_one(&pool)
    .await
    ...
    assert!(
        ...
        "Net PnL after fees should be +0.0983 SOL, got {}", net_stored
    );
}
...
async fn test_trade_uuid_exists_checks_dead_letter_queue() {
    ...
    insert_dead_letter(&pool, Some(uuid), "{}", "test reason", None, None).await.unwrap();

    let exists_dlq = trade_uuid_exists(&pool, uuid).await.unwrap();
    assert!(exists_dlq, "UUID in DLQ must be detected by trade_uuid_exists()");
}
...
async fn test_full_trade_status_progression() {
    ...
    insert_trade(&pool, uuid, "wallet", "token", None, "SHIELD", "BUY",
        Decimal::from_str("1.0").unwrap(), "PENDING").await.unwrap();
    ...
    for (status, sig) in [
        ...
        update_trade_status(&pool, uuid, status, sig, None).await.unwrap();
    ...
    // Open position
    open_position(&pool, uuid, "wallet", "token", None, "SHIELD",
        Decimal::from_str("1.0").unwrap(), Decimal::from_str("50.0").unwrap(), "sig123")
        .await.unwrap();

    // Close position
    close_position(&pool, "token", "wallet", Decimal::from_str("60.0").unwrap(), "sig_exit")
        .await.unwrap();

    update_trade_status(&pool, uuid, "CLOSED", Some("sig_exit"), None).await.unwrap();

    let (final_status,): (String,) = sqlx::query_as("SELECT status FROM trades WHERE trade_uuid = ?")
        .bind(uuid).fetch_one(&pool).await.unwrap();
    assert_eq!(final_status, "CLOSED");

    let (pos_state,): (String,) = sqlx::query_as("SELECT state FROM positions WHERE trade_uuid = ?")
        .bind(uuid).fetch_one(&pool).await.unwrap();
...
async fn test_failed_trade_can_retry() {
    ...
    update_trade_status(&pool, uuid, "FAILED", None, Some("RPC timeout")).await.unwrap();
    update_trade_status(&pool, uuid, "RETRY", None, None).await.unwrap();
    update_trade_status(&pool, uuid, "EXECUTING", None, None).await.unwrap();
