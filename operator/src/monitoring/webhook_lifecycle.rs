...
use crate::monitoring::helius::{
    HeliusClient, WebhookReconciliationDetail, WebhookReconciliationResult, WebhookUpdate,
};
...
impl WebhookLifecycleManager {
    ...
    pub async fn register_wallet_webhook(&self, wallet: &str) -> Result<WebhookRegistrationResult> {
        ...
        //
        // Self-healing: a candidate webhook can be stale (ID in DB but
        // deleted in Helius — e.g. by a cleanup race). When that happens,
        // clear all DB references to the stale ID so it is never picked
        // again, then retry with the next candidate before falling through
        // to creating a new webhook.
        for _attempt in 0..3 {
            let batch_webhook_id = match self
                .db
            ...
            if webhook.wallet_addresses.contains(&wallet.to_string()) {
            ...
            match self
                .helius_client
                .update_webhook(&batch_webhook_id, update)
                .await
            {
                Ok(()) => {
                    if let Err(e) = self
                ...
                Err(e) => {
                    warn!(
                        ...
                        webhook_id = %batch_webhook_id,
                        error = %e,
                        "Failed to add wallet to batched webhook — trying next candidate"
                    );
                }
            }
        }
            ...
            .await
        {
            Ok(webhook_id) => {
                ...
                if let Err(e) = self
                    .db
                    .upsert_wallet_monitoring(wallet, Some(&webhook_id), true)
                    .await
                {
            ...
            Err(e) => {
                ...
                let _ = self
                    .db
                    .increment_webhook_registration_attempts(wallet, Some(&e.to_string()))
                    .await;
    ...
    pub async fn reconcile_webhooks(&self) -> Result<ReconciliationResult> {
        ...
        let db_webhooks = self
            .db
            .get_all_wallet_monitoring()
            .await
    ...
    // Register missing webhooks
        ...
        if let Ok(Some(configured_url)) = self
            .db
            .get_webhook_configuration("current_webhook_url")
            .await
        {
            if configured_url != self.config.webhook_url {
                ...
                let updates: Vec<(String, String)> = db_webhooks
                    ...
                    .filter_map(|dw| {
                        dw.helius_webhook_id
                            .clone()
                            .map(|id| (id, self.config.webhook_url.clone()))
                    })
                    .collect();
    ...
    pub async fn health_check_webhooks(&self) -> Result<HealthCheckResult> {
        ...
        // Get stale webhooks
        let stale_wallets = self
            .db
            .get_stale_webhook_wallets(self.config.stale_threshold_days as i32)
            .await?;
        ...
        for wallet in &stale_wallets {
            ...
            match self.check_webhook_health(wallet).await {
                ...
                Ok(false) => {
                    ...
                    let _ = self
                        .db
                        .update_webhook_health_status(wallet, "unhealthy", None)
                        .await;
                }
                Err(e) => {
        ...
        if self.config.auto_cleanup_enabled {
            ...
            for wallet in unhealthy_wallets {
                match self.cleanup_wallet_webhook(&wallet).await {
                    Ok(()) => cleaned_up += 1,
                    Err(e) => {
                        warn!(wallet = %wallet, error = %e, "Failed to cleanup unhealthy webhook")
                    }
                }
            }
        }
    ...
    pub async fn reconcile_with_helius_dashboard(&self) -> Result<WebhookReconciliationResult> {
        ...
        info!(
            ...
            "Processing Helius webhooks for profitability"
        );
        ...
        match self.db.get_wallet(wallet_address).await {
            Ok(Some(wallet)) => {
                ...
                let confidence = if trade_count >= 20 {
                    ...
                    rust_decimal::Decimal::from_f64_retain(trade_count as f64 / 20.0)
                ...
                if confidence
                    < rust_decimal::Decimal::from_f64_retain(0.70)
                        .unwrap_or(rust_decimal::Decimal::ZERO)
                {
                    return Ok(WebhookEligibility {
                        eligible: false,
                        ...
                        roi_30d: wallet.roi_30d,
                        reason: format!(
                            "Confidence {:.2} below minimum 0.70",
...
mod tests {
    ...
    fn test_is_valid_solana_address() {
        ...
        assert!(is_valid_solana_address(
            "52kpqW23KVMhAC5Lnt5AWRQ63AQtGxrC7paiC5ttf9Tz"
        ));
        assert!(is_valid_solana_address(
