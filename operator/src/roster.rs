...
pub async fn merge_roster(pool: &Pool<Sqlite>, roster_path: &Path) -> AppResult<MergeResult> {
...
pub async fn validate_roster(pool: &Pool<Sqlite>, roster_path: &Path) -> AppResult<bool> {
    if !roster_path.exists() {
        return Ok(false);
    }
