...
ALTER TABLE wallet_monitoring ADD COLUMN registration_attempts INTEGER DEFAULT 0;
ALTER TABLE wallet_monitoring ADD COLUMN last_registration_error TEXT;
ALTER TABLE wallet_monitoring ADD COLUMN last_updated_url TEXT;
...
CREATE INDEX idx_wallet_monitoring_webhook_status
    ON wallet_monitoring(webhook_status) WHERE webhook_status = 'active';

CREATE INDEX idx_wallet_monitoring_health_check
    ON wallet_monitoring(webhook_last_health_check);

CREATE INDEX idx_wallet_monitoring_helius_webhook_id
...
CREATE INDEX idx_webhook_lifecycle_audit_wallet ON webhook_lifecycle_audit(wallet_address, created_at DESC);
CREATE INDEX idx_webhook_lifecycle_audit_action ON webhook_lifecycle_audit(action, created_at DESC);
CREATE INDEX idx_webhook_lifecycle_audit_status ON webhook_lifecycle_audit(status, created_at DESC);
