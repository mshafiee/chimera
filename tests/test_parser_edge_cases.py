...
class TestTransactionParser:
    ...
    @pytest.mark.asyncio
    async def test_parse_token_to_token_swap_stub(self, analyzer):
        """
        ...
        # Mock liquidity provider sol price for conversion (must be async since code uses await)
        async def mock_get_sol_price():
            return 100.0  # $100 per SOL
        analyzer.liquidity_provider.get_sol_price_usd = mock_get_sol_price

        trade = await analyzer._parse_swap_to_trade(swap_data, wallet)
