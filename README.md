...
[![Version](https://img.shields.io/badge/version-v1.0.1-blue)](https://github.com/mshafiee/chimera)
...
### 🔁 Scout Integration (Roster Merge)

The **merge_roster** utility integrates wallet discoveries from Scout into the Operator's database. After Scout analyzes wallets and generates `roster_new.db`, use this tool to merge the results.

#### Running merge_roster

**Option 1: Direct merge with default paths**
```bash
cd operator
cargo run --release --bin merge_roster
```

**Option 2: With custom paths**
```bash
cargo run --bin merge_roster -- \
  --roster-path /path/to/roster_new.db \
  --db-path /path/to/chimera.db
```

**Option 3: Using the compiled binary (faster)**
```bash
./target/release/merge_roster \
  --roster-path data/roster_new.db \
  --db-path data/chimera.db
```

#### What merge_roster Does

The merge utility performs the following operations:

1. **Reads** wallet roster from `roster_new.db` (generated by Scout)
2. **Validates** schema compatibility and data integrity
3. **Merges** new wallets into `chimera.db` with proper categorization:
   - `ACTIVE` → High-quality wallets ready for copy-trading
   - `CANDIDATE` → Promising wallets needing more data
   - `REJECTED` → Wallets that don't meet quality thresholds
4. **Updates** existing wallet statuses based on new analysis
5. **Removes** wallets that no longer meet criteria
6. **Reports** merge statistics and warnings

#### Output Example

```
=== Chimera Roster Merge ===
Roster file: data/roster_new.db
Database: data/chimera.db

Starting roster merge...
...
#### Triggering Merge from Running Operator

If the Operator is already running, you can trigger roster merge via:
...
**Signal (Unix):**
```bash
kill -HUP $(pgrep chimera_operator)
```

#### Scout Workflow Integration

1. **Scout analyzes wallets** → Generates `roster_new.db`
2. **merge_roster runs** → Integrates wallets into Operator
3. **Operator updates** → New wallets become available for copy-trading

This separation allows Scout to run independently (via cron) while the Operator continues trading with the existing roster.
