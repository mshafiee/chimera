...
def _load_promotion_criteria(args) -> PromotionCriteria:
    return PromotionCriteria(
        ...
        min_close_ratio_scalper=float(os.getenv("SCOUT_MIN_CLOSE_RATIO_SCALPER", "0.10")),
        min_close_ratio_swing=float(os.getenv("SCOUT_MIN_CLOSE_RATIO_SWING", "0.30")),
        min_close_ratio_whale=float(os.getenv("SCOUT_MIN_CLOSE_RATIO_WHALE", "0.25")),
        ...
        fast_track_min_trades=int(os.getenv("SCOUT_FAST_TRACK_MIN_TRADES", "5")),
    )
...
async def main_async():
    ...
    # top N CANDIDATE wallets by WQS directly via the validator.
    ...
    if _reval_enabled and not args.dry_run and validator is not None:
        ...
        for candidate in existing_candidates:
            ...
            try:
                ...
                if result.passed:
                    ...
                    reval_promoted += 1
                    print(f"[Scout] ✓ Promoted {addr[:8]} → ACTIVE ({result.reason})")
                else:
                    print(f"[Scout]   {addr[:8]} still failed: {result.reason}")
            except Exception as e:
