...
echo "----------------------------------------"
git fetch origin main > /dev/null 2>&1 || echo "Warning: git fetch failed; comparing against local refs only"
...
if [ -n "$LOCAL" ] && [ -n "$REMOTE" ] && [ "$LOCAL" != "$REMOTE" ]; then
    ...
    echo "Latest:  $(git log --oneline -1 origin/main 2>/dev/null || echo "unknown")"
    DIRTY=$(git status --porcelain 2>/dev/null || true)
    if [ -n "$DIRTY" ]; then
        echo "⚠️ Working tree is dirty (uncommitted changes may affect the running deployment)"
...
echo "----------------------------------------"
if docker compose -f docker-compose.yml -f docker-compose-haproxy.yml logs operator --tail=100 2>&1 | tail -50; then
...
echo "----------------------------------------"
if docker compose -f docker-compose.yml -f docker-compose-haproxy.yml logs scout --tail=100 2>&1 | tail -50; then
...
echo "----------------------------------------"
echo "Operator WQS Minimum: $(docker compose -f docker-compose.yml -f docker-compose-haproxy.yml config | grep CHIMERA_SELECTION__MIN_WQS_SCORE || echo "not set")"
echo "Scout Close Ratio SCALPER: $(docker compose -f docker-compose.yml -f docker-compose-haproxy.yml config | grep SCOUT_MIN_CLOSE_RATIO_SCALPER || echo "not set")"
echo "Scout Close Ratio SWING: $(docker compose -f docker-compose.yml -f docker-compose-haproxy.yml config | grep SCOUT_MIN_CLOSE_RATIO_SWING || echo "not set")"
echo "Scout Close Ratio WHALE: $(docker compose -f docker-compose.yml -f docker-compose-haproxy.yml config | grep SCOUT_MIN_CLOSE_RATIO_WHALE || echo "not set")"
echo ""
...
if [ ! -f /opt/chimera/operator/target/debug/chimera_operator ]; then
    echo "Note: debug operator binary not found (production hosts typically use target/release)"
fi
echo ""
...
echo "----------------------------------------"
echo "Net SOL flow by wallet:"
docker compose -f docker-compose.yml -f docker-compose-haproxy.yml exec -T postgres psql -U chimera -d chimera_db -c "
SELECT
    wallet_address,
    COALESCE(SUM(CASE WHEN action = 'BUY' THEN -amount_sol ELSE amount_sol END), 0) as net_sol_flow,
    COUNT(*) as trade_count,
    MAX(created_at) as last_trade
FROM trade_history
WHERE created_at > NOW() - INTERVAL '12 hours'
GROUP BY wallet_address
ORDER BY net_sol_flow DESC
LIMIT 10;
" 2>&1 || echo "Database query failed"
echo ""
...
echo "----------------------------------------"
LOG_ERRORS=$(docker compose -f docker-compose.yml -f docker-compose-haproxy.yml logs operator --tail=200 2>&1 | grep -i "error\|reject\|fail" || true)
if [ -n "$LOG_ERRORS" ]; then
    echo "$LOG_ERRORS" | tail -20
else
    echo "No recent errors found in operator logs"
fi
echo ""
