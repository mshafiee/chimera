...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-17 (commit fb31813). Confidence: 99%.
...
- `scout/main.py` — 57 bug fixes, last fix yesterday (bug magnet); 50 commits/90d
- `scout/core/helius_client.py` — 49 bug fixes, last fix 6 days ago (bug magnet); 37 commits/90d
- `scout/core/analyzer.py` — 49 bug fixes, last fix 2 weeks ago (bug magnet); 31 commits/90d
- `operator/src/engine/executor.rs` — 44 bug fixes, last fix 12 days ago (bug magnet); 18 commits/90d
- `operator/src/engine/signal_pipeline.rs` — 25 bug fixes, last fix 2 days ago (bug magnet); 33 commits/90d

### Code health
Three co-equal signals: defect risk 6.49/10 avg, hotspot health 3.09/10 (stable), worst `infra/src/db_abstraction/postgres.rs` at 1.0/10 · maintainability 7.49/10 · performance risk 206 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `core/src/config.rs` — change entropy — impact −2.8
...
- `config/config.yaml` — untested hotspot — impact −2.0
