...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-09 (commit ee8ecf4). Confidence: 99%.
...
### Architecture
Chimera is a high-frequency, fault-tolerant copy-trading platform for Solana: it consumes wallet trade signals over an HMAC-authenticated webhook, validates and executes them through a Rust hot path (token-safety checks, circuit breakers, Jito bundle submission, position tracking), and continuously refreshes its tracked-wallet roster from a Python cold path that scores candidate wallets (Wallet Quality Score) and backtests them before promotion — with a React dashboard as the monitoring and control surface. Two strategies run on the same engine: **Shield** (capital preservation, strict stop-losses and liquidity checks) and **Spear** (high-conviction, asymmetric upside via Jito bundles). Operationally it is a single-owner monorepo (881 files, ~254k LOC, bus factor 1) with heavy recent churn concentrated in scout/main.py, scout/core/helius_client.py, and scout/core/analyzer.py. Chimera is a **hot/cold split**.

### Key modules
- `web/src` — The web client is the operator-facing dashboard of the chimera system, hosting this subsystem's entry points: main.tsx and App.tsx…
- `scout/core` — Wallet intelligence lives here: scout/core is where Scout turns raw Solana on-chain data into wallet-quality decisions, fetching wallet…
- `operator/src` — High-frequency copy-trading on Solana is decided and executed here: the Operator scores watched on-chain signals, guards them with a…
- `core/src` — Everything that runs in the Chimera Operator imports core/src before anything else: the api service, the infra shared layer, and the…
- `web/src/api` — Every value the Chimera dashboard renders — wallet balances, open positions, consensus signals, risk gauges, live health checks — crosses…
- `web/src/components/ui` — The dashboard's presentational vocabulary is defined here: a single-maintainer set of building blocks — Button, Card, Badge…
- `root` — Application Bootstrap is where a running Chimera Operator process comes from: api/src/main.rs constructs the Axum HTTP service with…
- `operator/src/engine` — The operator's execution engine turns vetted signals into live Solana trades and then owns each position until it exits — sizing it by…
- `infra/src` — Infra owns the operator's persistence and its links to the outside world: the database writes for trade and position state, the…
- `infra/src/notifications` — The Chimera operator's alerting and pre-trade safety live here: notifications go out over Telegram and Discord when the circuit breaker…
...
- `scout/main.py` — 55 bug fixes, last fix today (bug magnet); 47 commits/90d
- `scout/core/analyzer.py` — 49 bug fixes, last fix 5 days ago (bug magnet); 31 commits/90d
- `scout/core/helius_client.py` — 46 bug fixes, last fix today (bug magnet); 36 commits/90d
- `operator/src/engine/executor.rs` — 44 bug fixes, last fix 3 days ago (bug magnet); 17 commits/90d
- `scout/core/wqs.py` — 30 bug fixes, last fix 5 days ago (bug magnet); 11 commits/90d

### Code health
Three co-equal signals: defect risk 6.51/10 avg, hotspot health 2.99/10 (stable), worst `operator/src/engine/mod.rs` at 1.0/10 · maintainability 7.6/10 · performance risk 189 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `operator/src/handlers/profitability.rs` — complex conditional (evaluate_gates) — impact −2.5
- `web/src/pages/Login.tsx` — churn risk — impact −2.4
